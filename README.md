...
**Keyboard:** W throttle (ramps up while held), A D / ← → steer (ramped, so a tap is a nudge and a hold is full lock), S / ↓ lean back (nose up — in the air, pitch up), Q / ↑ lean forward (nose down), SHIFT held is the TUCK (down behind the bars: a fifth off the drag area, worth about a km/h flat and calm and a great deal more into a head wind, at the cost of most of what you steer with your body — keyboard only, since a touch rider's thumbs are both busy), Space brake and reverse, R reset to the last gate passed, B restart the run, Enter take a screenshot, C camera (bow, nose, close, chase, far, heli — RESET, CAMERA and the SHUTTER are buttons under the minimap too), Escape pause. There is no gearbox, and the brake is not a pedal: Space drops the reverse BUCKET over the jet, which is the only way a watercraft slows itself and the only way it goes backwards. The stand-up carries no bucket, so on it the key does nothing.
