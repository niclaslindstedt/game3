...
Sea Haven is an arcade jet ski racer — no account, no download, free. The feel target is the 90s jetski racers with a modern look: the whole game is the sensation of a hull meeting a wave, so the water and the craft on it are grounded in published models rather than guessed. The sea is a sum of real waves built from the level's wind by a fetch-limited spectrum — half a metre of sea in the lee of the shore, growing the further out you ride, shoaling over the shallows and breaking where it gets too tall for the depth under it, and **no edge to the sea at all**: turn your back on the course and hold the throttle open and you ride straight out of the level into the open ocean, the wind freshening and the swell building the whole way into a twenty-metre storm — and the hull floats on that surface by displacement, lifts onto the plane as speed rises, feels the orbital push of every crest, decelerates hard on a hard landing and DIVES on a nose-down one. There is no brake pedal, no handbrake and no gear: a throttle, a nozzle, the reverse bucket that is the only brake a watercraft has, and a rider who leans. With no thrust there is almost nothing to steer with, so a turn is taken on the gas, which is the one thing to learn.
...
- **The water first.** The wave model is the game: Stokes-corrected waves off a JONSWAP spectrum, real dispersion, shoaling and breaking, a fetch law that grows the sea offshore, and an open ocean past the level's rim that has no far side. The renderer displaces its mesh by calling the SAME function the physics reads, so what is drawn is what is simulated.
...
| `make level`         | `SEED=38` — one level from above to `previews/level-<seed>.png`: depth shading, the shore, the solids, every gate numbered with its ramp, the wind arrow; plus a table of every gate with its offshore distance and depth. `TRACK=circuit` draws the ocean circuit that seed deals instead; `PACE=1.5` draws it at a speed class (R32); `RAMP=2` draws its ramps at R33's dial                                                                                                                     |
| `make analyze`       | Score generated levels: gates within 100 m of shore, depth along the path, solids clear of it, the run-up before every ramp, gate spacing and course length — exits non-zero on an error finding. `TRACK=circuit` scores circuits against R29–R31 instead; `PACE=1.5` scores a level against the book its class was drawn to; `RAMP=2` does the same for R33's ramp dial                                                                                                                           |
...
| `make birds`         | The birds lab: every bird on the coast side by side as one labelled contact sheet to `previews/birds.png` — each drawn gliding, mid-beat and perched with its wings folded over a metre rule, seen from below as the water sees it (`ARGS="--rows=gull,eagle"` for a slice)                                                                                                                                                                                                                        |
| `make hero`          | The poster lab: the game staged to be LOOKED at — the open ocean with the sun on it, the same water under the stars, a hull through a ring at the top of its arc — into `previews/hero/`, with the HUD off. Each row of its table says what it is for, and names where the sun stands as an ANGLE rather than an hour, so a shot means the same thing in every season (`ARGS=--list` prints the table, `SHOT=sundown` takes one, `ARGS="--og sundown"` writes the site's share card from it)       |
...
| `make wake`          | The wake lab: the trail from straight above, one column a moment along a scripted run, as one labelled contact sheet to `previews/wake.png` — the jet blasting astern off a standing start, the road catching up as the hull moves, the fan opening behind it. Every camera in the game is a chase camera, so this is the only view of the plan shape (`ARGS=--drive=carve` for the turn, `ARGS=--channels` for foam/churn/crest/hollow apart, `ARGS="--times=0.1,0.3,0.6"` for the first moments) |
| `make profile`       | Meter what one frame costs the renderer — draw calls, triangles and binds per scene                                                                                                                                                                                                                                                                                                                                                                                                                |
| `make icons`         | Regenerate the PWA icons and the favicon from the app mark — the wave of `pwa/src/game/app-mark.ts`, rasterized (`ARGS=--sheet` also writes a contact strip of every tile at 1:1, which is the only way to look at a 32-pixel favicon). The share card is not here: it is a photograph now, and `make hero ARGS="--og <shot>"` writes it                                                                                                                                                           |
...
**Keyboard:** W throttle (ramps up while held), A D / ← → steer (ramped, so a tap is a nudge and a hold is full lock), ↓ lean back (nose up — in the air, pitch up), Q / ↑ lean forward (nose down), SHIFT held is the TUCK (down behind the bars: a fifth off the drag area, worth about a km/h flat and calm and a great deal more into a head wind, at the cost of most of what you steer with your body — keyboard only, since a touch rider's thumbs are both busy), S / Space brake and reverse, R back to the last gate passed, B restart the run, Enter take a screenshot, C camera (bow, nose, close, chase, far, heli — RESET, CAMERA and the SHUTTER are buttons under the minimap too), Escape pause. **Every one of them is yours to change** — OPTIONS ▸ KEYBOARD: press a row, press the key. There is no gearbox, and the brake is not a pedal: it drops the reverse BUCKET over the jet, which is the only way a watercraft slows itself and the only way it goes backwards. The stand-up carries no bucket, so on it the key does nothing.
