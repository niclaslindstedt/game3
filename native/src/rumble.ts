 ...
 * should: a 26 ms slap off the chop and a 70 ms touchdown are one tap, a
 * rock met at speed is two or three, and the 260 ms capsize is three. */
const MS_PER_TAP = 85;

/** The most taps one pulse is ever worth. Going over is three; anything
 * longer would still be buzzing while the rider is back on the saddle. */
const MAX_TAPS = 3;

/** Where the strength bands fall, 0..1. The light band is deliberately wide:
 * the chop's slap lives in most of it, and it is meant to be felt as a
 * texture under the palms rather than noticed as a buzz. */
 ...
 * hardware with none: a slap off a chop is one tap and the hull going over
 * is three, so the difference between them is felt as length even though no
