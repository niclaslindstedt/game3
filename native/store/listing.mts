...
export const RULES: StoreRules = {
  ...
  apple: {
    // Racing first because that is what it is; SPORTS second because a jet
    // ski on a timed course is a sport before it is a simulation, and the
    // people who want one are looking in that aisle.
    categories: [["GAMES", "GAMES_RACING", "GAMES_SPORTS"], "SPORTS"],

    advisory: {
      // NONE, and unusually it is the easy answer. A craft that touches a
      // buoy is pushed off it and loses time (engine/game/collision.ts —
      // `push` and `penalty`); nothing is damaged, nobody is thrown, and
      // there is no one else on the water to hit. The rider is never drawn
      // coming off.
      ...
      // Nothing is wagered. The shore generator draws from a seed, which is
      // not a wager, and there is nothing to stake on the outcome.
      ...
      // elsewhere: the HUD's build label hands its one link to Safari.
      unrestrictedWebAccess: false,
      kidsAgeBand: null,
    },

    // A name and a mailbox, not a phone number. Committed because they are
    // the publisher's public contact details, which the listing shows anyway.
    contact: {
      firstName: "Niclas",
      lastName: "Lindstedt",
