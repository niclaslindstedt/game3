...
Sea Haven ships on the **App Store** alone. The desktop shell under `tauri/`
has no Mac App Store record and there is no Steam page, so `listing.mts` says
so and neither the generator nor the preflight asks for them. Turning one on is
the flag plus the section it belongs to — the shape is in `game2`, which ships
on all three.

The screenshot harness is NOT ported: its recipes stage the actual game, and
Sea Haven's have to be written against its own HUD and shores rather than
inherited.
