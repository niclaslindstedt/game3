...
for (const [page, why] of [
  ...
  if (existsSync(file)) ok(`/${page}/ has a page behind it (${rel(file)})`);
  else fail(`the listing names /${page}/ but ${rel(file)} does not exist`, why);
}
...
if (existsSync(at("pwa", "dist", "index.html"))) {
  ok("pwa/dist is built");
} else {
  warn(
    "pwa/dist has not been built",
