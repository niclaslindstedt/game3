...
// The app mark's wave is stated three times — as `d` strings in
...
import { MARK_WAVE, MARK_WIDTH } from "../pwa/src/game/app-mark.ts";
...
describe("the app mark", () => {
  it("draws the same two wave paths the icon SVG draws", () => {
    const paths = [...svg.matchAll(/<path[^>]*\sd="([^"]+)"/g)].map((m) => m[1]);
    expect(paths).toEqual([...MARK_WAVE]);
  });
