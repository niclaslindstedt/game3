...
- **HOW BIG THE SEA IS PAST THE RIM** — `TUNING.sea.open.ladder` and `stormRamp` (`engine/game/ocean.ts`): the storm's height rung by rung, from twenty metres a couple of kilometres out to a thousand two hundred kilometres past it, read straight between rungs. The SEA's height and the BED under it both climb on that one ramp, because the depth-limited clip couples them; the coast's own sea and the WIND both end on the shorter `coastAstern`, because both are the coast's weather stopping rather than the ocean's building. `createSea` lays a band per rung so each height keeps its own wavelength, and `stormSeaAt` is what the renderer judges a crest against out there.
