...
| How fast the whole roster goes: the speed class      | `TUNING.pump.speedClass`, `engine/game/limits.ts`             | `craft-tuning`       |
...
| What FLIES here: the roster — the gull, the tern, the cormorant, the eider, the eagle, and the geese, swans and cranes that cross | `pwa/src/game/bird-defs.ts` | `nature` |
| Where every flock LIVES, and where every bird IS at a moment: the roost, the loop, the cycle, the flush, the crossings | `pwa/src/game/bird-plan.ts` (three-free, so the tests read it) | `nature`, `atmosphere` |
...
- **HOW BIG THE SEA IS PAST THE RIM** — `jumpableHs(speed)` and `STORM_CEILING` (`engine/game/ocean.ts`): the biggest sea the roster's FASTEST craft can still fly over the rim of and down to the floor of, which is a closed form QUADRATIC in the top speed (a wave's width grows with its height, a flight's reach does not, so the two cross once). Nothing anywhere quotes an ocean height: turn `TUNING.pump.speedClass` up and the ocean grows with the square of it. Each level then DEALS its own storm over the top `open.vary` of that ceiling, so the biggest is rare — one seed in ten. `createSea` lays a band per rung of `open.rungs` so each height keeps its own wavelength, and `stormSeaAt` is what the renderer judges a crest against out there.
- **HOW FAST A CRAFT CAN GO** — `topSpeedOf` and `accel0to50Of` (`engine/game/limits.ts`), which are the ONE place `TUNING.pump.speedClass` is applied; `classPitch` / `classTorque` beside them are the two factors the pump reads. The catalog's `topSpeed`, `accel0to50`, `powerKw` and torque curve are the hull's own numbers AT CLASS 1 — what separates the four must not move when a class does.
...
- **The rule book has a mirror, and TWO CHAPTERS.** `engine/mapgen/rules.ts` (R1–R28, a coast sprint) and `engine/mapgen/rules-circuit.ts` (R29–R31, the ocean circuit that replaces R1, R10, R25 and R26 on a level drawn as one) each state their rules once in the file header; `docs/level-generator.md` carries the same prose VERBATIM for both. `tests/docs_rules_test.ts` reads the ids off both files, so a new rule fails the test until its mirror lands. The split is the §20.5 cap, not two rule books: `LEVEL_RULES.circuit` is the second chapter's table and everything reads the numbers through the one `LEVEL_RULES`.
