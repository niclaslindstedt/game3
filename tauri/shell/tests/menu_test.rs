...
//! never fires), a binding that takes a key the CRAFT needs, a row pointing at
...
#[test]
fn the_first_menu_is_the_app_and_the_second_is_the_ride() {
    let bar = menu_bar();
    ...
    // Cut/Copy/Paste, and ⌘C is dead on the developer page's repro link.
...
/// away from the craft for the life of the window — silently, and only on
/// macOS. Every binding therefore carries a modifier.
#[test]
fn no_row_takes_a_key_off_the_craft() {
