...
`cargo test -p seahaven-shell` therefore runs the entire decision layer on a
...
normally; the repository link on the developer page and any credit open in the
...
(`%APPDATA%\seahaven` on Windows, `~/Library/Application Support/seahaven` on
macOS, `~/.local/share/seahaven` on Linux), keeping the previous one beside it
...
The sibling rally repo's shell also carries a **store half** — a Steam page, a
