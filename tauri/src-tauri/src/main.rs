...
//!     the event-loop library unwraps a missing one (`seahaven_shell::display`).
...
use seahaven_shell::config::{remote_game_url, user_data_dir, APP_SCHEME, SHELL_NOTICE};
use seahaven_shell::output;
use seahaven_shell::webroot::webroot_exists;
...
pub struct Shell {
    ...
    /// belongs to the webview; see `seahaven_shell::config::APP_DIR_NAME`).
...
fn install_panic_hook() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        ...
        output::error(&seahaven_shell::display::panic_report(
            &payload,
...
fn main() {
    ...
    if let Some(refusal) =
        seahaven_shell::display::refuse_windowless(std::env::consts::OS, &|name| {
            std::env::var(name).ok()
        })
    {
    ...
    tauri::Builder::default()
        ...
        // Every menu row this app declared, spent by `seahaven_shell::menu`'s
        // table.
