...
//! `seahaven_shell::window_state` and of `seahaven_shell::config`.

use std::path::{Path, PathBuf};
use std::sync::atomic::{AtomicBool, Ordering};

use seahaven_shell::config::{is_internal_url, remote_game_url, start_url, BRAND_BG, WINDOW_TITLE};
use seahaven_shell::output;
use seahaven_shell::window_state::{
...
pub fn app_origin() -> String {
    let scheme = seahaven_shell::config::APP_SCHEME;
    let host = seahaven_shell::config::APP_HOST;
    if cfg!(windows) {
...
/// the repository link on the developer page, a credit — opens in the player's
