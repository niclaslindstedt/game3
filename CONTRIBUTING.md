# Contributing to Sea Haven

Thanks for wanting to make the water better. This document covers everything from a clean checkout to a merged PR. By participating you agree to the [Code of Conduct](CODE_OF_CONDUCT.md); security problems go through [SECURITY.md](SECURITY.md), never public issues.
...
make test        # vitest: waves, buoyancy, craft, flight, the generator, bot simulations
                 # (CI slices it four ways with SHARD=i/4; locally, run the files that cover the change)
...
1. **Fork and branch.** Branch from `main`, named `<type>/<short-topic>` — e.g. `feat/wake-spray`, `fix/ramp-launch-pitch`.
2. **Change the code.** Where things go is mapped in [AGENTS.md](AGENTS.md) ("Where new code goes") — the short version: the water, the craft and the course in `engine/`, rendering in `pwa/src/game/`, never physics in the renderer.
3. **For hull, water or generator changes, run the labs before and after.** `make sim` prints the pace / air / gates table; `make waves` and `make ride` draw the water and the craft on it. Paste both tables in the PR description. A change that makes the bot stop finishing courses, or a craft that stops floating at the draft its mass implies, is a regression until argued otherwise.
4. **Add or update tests.** Physics moments (the launch, the landing, the dive, the carve) and generator rules are test-covered; a behavior change without a matching test change is incomplete. Tests live in `tests/*_test.ts` (see AGENTS.md → Test conventions).
5. **Drop a changeset fragment** for anything user-visible: a file in `.changes/unreleased/` named `<unix-ts>-<slug>.md`:
   ...
   A hull on the plane now throws a proper rooster tail.
   ```
...
6. **Update docs** the change touches (`docs/water.md` for the wave model, `docs/riding.md` for the feel, `docs/level-generator.md` for rules, the README's Usage table for new commands).
7. **Commit** with [conventional commits](https://www.conventionalcommits.org/): `feat(engine): give the hull a rooster tail`. Types: `feat fix docs refactor test build ci chore perf style revert`; breaking changes use `!` or a `BREAKING CHANGE:` footer. The `commit-msg` hook enforces this.
...
- **Bugs & feature requests** → [GitHub Issues](https://github.com/niclaslindstedt/game3/issues) (use the templates)
- **Questions & ideas** → [GitHub Discussions](https://github.com/niclaslindstedt/game3/discussions)
