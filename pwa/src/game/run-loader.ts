...
//
// Standing a shore up is the most expensive thing this game does, and almost
// none of it is the water: the generator searches its sub-seeds, lays a
// coast, runs a course along it, scatters the skerries and BAKES TWO WHOLE
// HEIGHTFIELDS (`mapgen/compile.ts`), and only then does the renderer get to
// build a terrain mesh off one of them, stand the rocks and the gates up, and
// hand the whole scene to a driver that has to compile every shader in it.
//
...
// nothing to say the press has landed; left to arrive during the run, the
...
// lifts the shore is compiled, the scene is built and the shaders are
// compiled. The run then costs what a run costs.
//
...
// (`tests/run_loader_test.ts`). The steps themselves are closures over the
// app's own refs, built in `App.tsx`, and each brings the words the card says
...
export type LoadStep = {
   ...
   * rocks stood up out of a field, gates built out of a course. Left
   * off by the steps that cannot: compiling a shore and building the water
   ...
   * (standing the rocks up, a handful at a time) says so.
   *
   * A step that CANNOT be cut up — compiling the shore, building the water mesh —
 ...
 * "more to do" is therefore not a yield: the rock builder says it after every
