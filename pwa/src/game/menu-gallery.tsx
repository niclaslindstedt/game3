...
// sent on from there. Reached from the front door's GALLERY row, and the
// other half of the shutter: ENTER files a picture (screenshots.ts) and
// this is the only place one is ever shown.
//
...
// decodes on the frame the player pressed — on the same thread the sea
...
export function GalleryPage({ onBack }: { onBack: () => void }) {
  ...
  // taken again — the shore it was taken on has long since been rebuilt.
  ...
  return (
    <div class="menu-card menu-card-gallery">
      ...
      <MenuHead
        ...
        sub={shots.length === 0 ? undefined : STRINGS.gallerySub(shots.length, MAX_SHOTS)}
      />
      {shots.length === 0 ? (
