...
/** The heading swung from `from` to `to`, rad, the short way round: a trail
 * that crosses the wrap is still a gentle turn. */
function turned(to: number, from: number): number {
...
export function createWake(): Wake {
  ...
  // How far along the trail the sample sits, m — what the fan's cusps are
  // anchored to, so the scallops stand still in the water while the craft
  // runs away from them.
  const sr = new Float32Array(SAMPLES);
  // …and how hard it was turning when it was laid, rad/s — which side of
  // the fan is the outside of the carve.
  ...
  const push = (
    ...
    run = last >= 0 ? sr[last] + Math.hypot(x - sx[last], z - sz[last]) : 0;
    // How hard the hull was turning over the stretch this sample closes:
    // the heading it actually swung through, the short way round, over the
    // time it took. A gap row carries nothing — a hull in the air is not
    // carving water.
    const span = last >= 0 ? t - st[last] : 0;
    sy[head] = !isGap && last >= 0 && span > 1e-4 ? turned(heading, sh[last]) / span : 0;
    sx[head] = x;
    sz[head] = z;
    sh[head] = heading;
    st[head] = t;
  ...
  const observe = (state: GameState): void => {
    ...
    switch (trailAction(now.live, end, moved)) {
      case "none":
        return;
      // Out of the water, or stopped: one dead row closes the trail at the
      // transom, so the last real row fades out where it ended.
      case "close":
        push(now.x, now.z, c.heading, state.t, 0, 0, now.beam, true);
        return;
      // Back in the water after a flight or a reverse: a dead row HERE
      // before the first live one, so the void the hull flew over is spanned
      // by two dead rows and not by a wedge of road stretched across it.
      case "open":
        push(now.x, now.z, c.heading, state.t, 0, 0, now.beam, true);
  ...
  const row = (
    ...
    // carve — which is why the half-width is asked for per side rather than
    // once for the row.
    const live = !dead && age < FAN_LIFE;
    for (let a = 0; a < FAN_S.length; a++) {
      const s = FAN_S[a];
      const bias = turnBias(s, turn);
      const fHalf = live ? fanHalf(beam, speed, age, along, bias) : 0;
      if (fHalf > 0) fanAt(s, age, speed, strength, section, along, bias);
      else section.cover = 0;
