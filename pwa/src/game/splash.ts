...
// is a LIVE SEA: the render stack has to come down the wire, the generator
// has to build a whole shore and the water beside it, and the bot has to
// start riding it. All of that is spent BEHIND the card.
//
// It plays like an arcade cabinet, in two beats. First the house's name over
// an empty sky while the sea is built. Then, the moment the game is standing,
// the app's own wave draws itself, SEA HAVEN fades in under it, and the card
// asks for a press — and waits, however long that takes. Nothing lifts the
...
// a sound — which matters the day `game/audio/` stops being a placeholder.
//
// Kept apart from the component so the timing rules are testable without a
// renderer (`tests/splash_test.ts`), which is also why this module takes the
 ...
 * first shore standing, the loop turning.
 *
 * **THE READY STATE WAITS FOR THE LOAD.** A card that invited a press while
 * the sea was still being built would hand the player exactly the empty blue
 ...
 * screenshot tool and the pinned runs (`?start=1`, `?scene=`, `?shot=`) want
 * the game, not the house's name. `?splash=1` forces it back for looking at
 * the card itself; `?splash=0` clears it off an ordinary visit, which is what
 * the developer page's repro links carry.
 */
export function splashSkipped(search: string): boolean {
  ...
  return params.get("start") === "1" || params.get("shot") === "1" || params.get("scene") !== null;
}
