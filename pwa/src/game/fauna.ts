...
export function createFauna(level: Level): Fauna {
  ...
  // R37 — and on a frozen run only the pods whose water is the channel's:
  // an animal under two metres of ice is drawn for nobody, and a seal
  // rising through the sheet is a fault.
  const pods = level.fauna.filter((pod) => iceAt(level, pod.x, pod.z) < 0);
