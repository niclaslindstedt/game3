...
export { rarestPod, shotFlock } from "./scenario-wildlife.ts";
...
export function scenarioFor(state: GameState, name: ScenarioName): Scenario {
  ...
  switch (name) {
    ...
    case "drift": {
      // LYING TO, WELL OUT: the engine off and the hull left to the sea.
      // Every other mark on the water is one the craft's PASSAGE left, so
      // this is the only moment where the sea carries nothing but the
      // rings the hull's own heave has radiated (`wake-bob.ts`) — a train
      // of them standing in the water the craft has drifted away from,
      // held long enough for three or four to be up at once. Out past the
      // course, where the sea is itself rather than in the start's lee.
      const at = outToSea(level, mid.x, mid.z, 120);
      const heading = level.wind.from + Math.PI / 2;
      return {
        moment: { x: at.x, z: at.z, heading, nextGate: mid.index },
        script: () => NEUTRAL,
        seconds: 10,
      };
    }
