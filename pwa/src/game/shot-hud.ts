...
//   the picture has to carry the clock, the gate count and the wind that
...
// the canvas — the HUD, and any card sitting above it — because all of it
// was on screen when the button went down. The one child left out is the
// CANVAS, which IS the picture.
 ...
 * the picture the layer is drawn over. */
const NOT_IN_LAYER = "canvas";

/**
 * The screen's chrome as it stands right now, or null when there is none to
 * take: the instruments are down (OPTIONS' HUD switch is off), or the
 * window has no size to speak of.
 *
 * That switch is the reason the HUD has to be there before anything is read:
 * it is how a rider asks for a frame judged on its pixels, and a layer that
 * came back with the new-build mark in it would have missed the point of the
 * press.
 */
