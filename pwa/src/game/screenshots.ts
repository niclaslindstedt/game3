...
//
// The mirror comes for free — it is a render pass, not DOM
// (`reflection.ts`) — so what the water is holding is part of the frame in
// the same way the hull is.
//
...
// drawn in Canvas2D would be a fourth thing to keep in step. Inlined at
// build time, so a picture taken offline is signed exactly like one taken
// online.
import iconSvg from "../../public/icons/icon.svg?raw";

/** How many pictures the roll keeps. Forty frames of a shore at 1920 wide
...
function drawStamp(
  ...
  // A bright sky, a wave's crest, a white hull: the frame under the stamp
