...
// sound design as diffable as the craft catalog.
//
...
// jet ski sounds like is a small engine with grit in it, a pump that whines,
// a sheet of spray that hisses, and water — which has no transient at all.
...
//     a rumble is what a swell, distant surf and a landing are all made of.
//     Pink sits between them and is the spray-and-wind band.
//   * NOISE HAS AN ENVELOPE. A chip's noise burst is a flat block that stops.
//     A splash swells, holds and thins out.
//   * FILTERS SWEEP. A static cutoff is a tone colour; a moving one is a
//     GESTURE — the sheet leaving the chines, a bow burying and coming back
//     up, the whoosh through a ring.
...
//
// AND THE BEDS ARE NOT ONE-SHOTS. The engine, the pump, the spray, the wind
// and the sea are LAYERS: node graphs built once and steered with smoothed
...
// instance per context keeps overlapping sounds in the same place. Open water
// reflects almost nothing back — what answers a sound out here is the SHORE,
// a couple of hundred metres off — so it is short, damped and only ever a
// send: a slap on the water is dry, and a hit on a skerry gets the rock.
...
// engine, a pump, the spray, the wind and whatever the hull just hit, all at
 ...
 * a hull throwing spray. Both are re-normalised, because an unnormalised brown
...
export function createSynth(): Synth {
  ...
  const ensure = (): AudioContext | null => {
    ...
    if (!ctx) {
      ...
      // main thread here is also displacing a sea.
  ...
  const applyFilter = (
    ...
    // Bluetooth headset that is every spray hiss in the game screaming.
