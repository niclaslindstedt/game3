...
that owns the subject (`engine-system`, `water-feel`, `craft-physics`,
...
- **Units and ranges on every tuning number** (metres, m/s, m/s², seconds,
  ...
  `defs/tuning.ts` or `defs/craft.ts` without a unit is a bug waiting for its
  next reader.
- **A physical model names its source.** Every force in `craft.ts`, `hull.ts`,
  `flight.ts` and every term in `water.ts` comes from a published model
  (Savitsky, ITTC-57, JONSWAP, McCowan…) — the comment above it says which,
...
- **Name the failure a rule prevents.** "Thrust vanishes with the intake out of
  the water — steering with it, which is what makes a landing pointed where the
  hull was thrown" beats "no thrust when airborne".
...
- **Fix inefficient algorithms on sight.** A needlessly bad complexity or a
  wasteful hot-path pattern (a linear scan over every gate each step where an
  index walk works, per-step allocation inside `step()` — it runs 120×/s, a
  per-vertex `surfaceAt` that rebuilds the wave components each call, per-frame
  allocation in the renderer or the water mesh's displacement loop) gets fixed
...
| craft / water / generator numbers | `npm run sim -- --seeds 1,2,3` — a slice, not the whole sweep |
| the wave model | `make waves SEED=1` — seconds, no build |
| the hull, the jet, the flight | `make ride SCENARIO=<one>` — seconds, no build |
...
- **No DOM, no browser, plain Node** — that is the actual line, not "engine
  ...
  several are tested that way (`scenarios_test.ts` over
  `pwa/src/game/scenarios.ts`, `input_model_test.ts` over `input-model.ts`):
  a staged moment is arithmetic, and a rule joining two layers has nowhere
...
- Physics tests stage a **synthetic level** — a hand-built `Level` with a flat
  sea bed at a chosen depth and a straight course of a few gates — handed to
  `createGame({ level })`, on a CALM sea (`wind: { from: 0, speed: 0 }`) when
  the wave field is not the subject, and script inputs step by step — see the
  `test-scenario` skill. Deepen the bed when a scenario dives or a landing
  reaches the bottom, so the physics is measured rather than the grounding.
...
- Assert the rule you claim. "No thrust out of the water" is a claim and owes
  an assertion.
...
- [ ] The code sits where `AGENTS.md`'s tables say it sits; the hard rules hold
      ...
      randomness only via the state's seeded RNG, the water is a pure
      function of `(x, z, t)`)
