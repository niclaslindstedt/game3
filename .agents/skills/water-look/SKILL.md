...
| `pwa/src/game/reflection.ts` | WHAT THE WATER MIRRORS BESIDES THE SKY: the scene drawn once a frame from the lens's mirror image in the water plane, Lengyel's oblique near plane so nothing under the surface is drawn, into a small texture the shader lays over the analytic sky wherever it has a picture. The DETAIL row's; OFF is no pass |
