...
one. A session that loaded `water-feel` and `simulate-run` owes two passes.
...
node scripts/skill-lessons.mjs mapgen-improvement --concepts=gates,ramps
...
concepts: [buoyancy, probes, draft]
---
...
| --- | --- | --- |
| `title` | yes | The gist in one line. Written as a CLAIM, not a topic — "Slamming damping must read the probe's OWN vertical closing speed, not the CoG's, or a flat landing dives", never "About landings". |
...
concepts (`wave`/`waves`, `sim`/`simulation`) get merged in the same pass.
