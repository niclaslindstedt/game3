---
name: commit
description: "Commit staged changes, push the branch, and create or update a PR with a conventional-commit-formatted title. Use after completing a feature or fix. Owns the quality-gate split (format, lint and the AFFECTED tests before the commit; the whole suite left to the PR's CI), the repo's commit and PR conventions, and the sim-table obligation on craft, water and generator PRs."
---
...
- **RUN THE TESTS FOR WHAT YOU WROTE. LET CI RUN THE REST.** CI shards the
  whole suite across four runners and runs it on every push whatever you do
...
- **Know when the blast radius is wider than the diff.** A change to
  `TUNING`, `engine/game/craft.ts`, `hull.ts`, `water.ts`, `engine/sim/`, or
  the generator moves numbers that tests three directories away assert on —
  a planing-lift retune moves the catalog's top-speed expectation in
  `craft_test`, the ramp launch in `flight_test`, the bot's finishes in
  ...
  (`simulation`, `determinism`, `scenarios`) plus the physics ones the change
  reaches (`craft`, `buoyancy`, `flight`, `waves`, `collision`, `course`).
- **`make sim` is not optional and is not the suite.** It is the table the PR
  owes on any craft, water or generator change, and no amount of CI replaces
  reading it yourself.
- **Craft, water or generator changes carry the `make sim` table, before AND
  after**, pasted into the PR description — and the owning lab's output beside
  it (`make ride SCENARIO=` for the hull, `make waves` for the sea,
  `make analyze` for the generator). This is the contract in CONTRIBUTING.md
...
not worth running serially here to learn what four parallel runners are about
to tell you.
...
`<type>/<short-topic>` in kebab-case (`feat/wake-spray`,
`fix/air-gate-miss`). In a remote/managed session, keep the harness-assigned
...
**Test plan** (commands run; the before/after `make sim` tables for craft,
water or generator changes, with the owning lab's strip or table beside them;
...
- Docs move with the code: check the "Documentation sync points" table in
  `AGENTS.md` before calling the branch done (`docs/water.md` for the sea,
  `docs/riding.md` for the craft, `docs/level-generator.md` for the rules,
