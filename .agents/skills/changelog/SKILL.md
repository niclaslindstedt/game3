...
rewrite of the entire water mesh that changes nothing on screen is
`no-changelog`; a one-word change to the HUD is a fragment. A generator rule
change is a fragment by definition: every level on every seed re-rolls.
...
A hard nose-down landing now buries the bow and throws the craft into a dive.

<!-- bad: three sentences, implementation detail, no player in it -->

Re-entry now applies Wagner slamming per bow probe scaled by v_z². The pitch
moment from the buried probes emits a dive event. This also fixes the silent
landing bug.
```
