---
title: A row that defers to the level marks the dealt answer and stores null — and only a DRIVEN browser can check it, because every screenshot catches the one state where the mark and the selection are the same chip
date: 2026-09-10
scope: pwa/src/game/menu-start.tsx, pwa/src/game/menu.tsx, pwa/src/game/seed-preview.tsx
...
answer that is rather than offering a chip for "whichever one it is":
`OptionRow` takes `dealt`, marks that chip, and a press on it stores null
again. Two things this needs that are easy to miss.
...
photograph the untouched card, where the marked chip IS the active one —
...
`.menu-opt-active` / `.menu-opt-dealt` per row plus
