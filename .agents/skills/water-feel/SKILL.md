---
name: water-feel
description: "Use when working on THE SEA — the wave field the hull rides (the Stokes-corrected component sum, its spectrum, its steepness), the sea state a level's wind and fetch build (the JONSWAP / Pierson–Moskowitz spectrum, the fetch-limited growth), how a wave changes coming ashore (dispersion, shoaling, breaking), the orbital and Stokes velocities the craft feels, the gusts, how the renderer's water mesh follows all of it, and the WASH the hulls leave in that water (`engine/game/wash.ts`: the V behind a moving hull, a landing's ring, a bobbing hull's ripples, as real water every probe reads and the grid draws). Owns `engine/game/water.ts` and `wind.ts`, `wash.ts`, `TUNING.sea` and `TUNING.wash`, `pwa/src/game/water-mesh.ts`'s displacement, `make waves` — the lab that must run before and after any change to the sea — and `make wash`, the same for the wash. Not the hull's answer to the water (`craft-physics`)."
---
