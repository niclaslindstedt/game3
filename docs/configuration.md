...
| `time`     | The start card's TIME row: `sunrise`, `day` or `sunset`, resolved by the engine against this coast's own daylight window (R13) in the season being ridden, rather than as three hours written down. `hour` wins where both are given.                                                                                                                                                                                                   |
...
| `detail`   | The picture's DETAIL row: `low`, `medium` or `high` — the spray off the hull and the wake behind it (foam alone, or the water bending for the craft as well), the sea life under it, how thickly the shore is planted, how many sheets of cloud are in the sky (and so in the sea reflecting it), how much rain is in the air and whether it lands on the water, and whether the shore and the craft are mirrored in it, as one choice. |
