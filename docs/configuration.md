...
| `track`    | `circuit` deals the seed as an OCEAN CIRCUIT (R29–R31): a closed lap out in open water, ridden two or three times round, with no river and the coast a long way off. Anything else (or nothing) is the default coast sprint. The HUD grows a LAP chip on a circuit and loses it on a sprint.                                                |
