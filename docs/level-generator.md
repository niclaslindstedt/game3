...
- **`biomes.ts` — the coasts.** What a level is built ON, as rows: the water's density and temperature band, how far north it lies, how high the land stands, how thickly the rocks lie, how much of the waterline is beach, how big a sea its wind grows and how much swell reaches it, which skies it can be under and what swims in it. Seven ids are reserved so a campaign location never changes its name; two rows are built — the taiga's, a cold sheltered skerry coast, and the mangrove's, a warm flat coast of white sand and turquoise water — and asking for another throws. A biome is a kind of coast, never a place.
