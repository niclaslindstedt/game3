...
- **R13** THE DAY AND THE WATER. The run is ridden at an hour inside `day.hour` — SOLAR time, read against the coast's own latitude (`Biome.latitude`, 62°N on the taiga coast), which is what decides where the sun actually stands at it and therefore what sky the run is under; the water's temperature comes from the biome's band and its density is the biome's (brackish 1005 kg/m³ on the taiga coast).
...
3. **The conditions.** Wind from the seaward normal ± 60°, at 6–14 m/s — a fresh breeze most days, a strong one on some; an hour anywhere on the clock; a water temperature from the biome's band.
