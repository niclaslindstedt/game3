...
3. **The components — THREE BANDS (R28).** There are three kinds of water a rider can reach and they do not carry the same waves, so the field is laid in three bands and every point takes a share of each.
   ...
   **The open bands** (`sea.components` again, one band per rung of the STORM LADDER) are the storm past the edge of the built level — [the open ocean](#the-open-ocean-past-the-rim-enginegameoceants) below, which owns the whole of it. Every one of their shares is 0 inside a level's bounds, so the coast's own water is untouched by them and the whole ladder costs one comparison a sample: `surfaceAt` walks the field BAND BY BAND and skips a band whose share is nothing in a single test rather than once per component.
...
`SeaState` carries `windSpeed`, `windFrom`, the level's `bounds` (where its grid, and so its coast, stops), the `shelter` the level was measured into, `fetchRef`, `hsRef`, `tp` (the ocean band's), `localHs`, `localTp` (the local band's), `openHs`, `openTp` (the TOP rung of the storm ladder's), the `bands`, and the components.

`bands` is every band of the field — the ocean's, the local one, and one per rung of the ladder — each with the height and period it was laid at and `at`, the positions of its components. It is what `surfaceAt` walks, in both of its passes, so a band standing at nothing costs one comparison instead of one per component: over a course that is the whole ladder, and it is why carrying six rungs of storm is cheaper than the single open band was (`surfaceAt` measures 557 ns a sample against 626 ns before it).
...
`seaShares(sea, x, z)` returns the three KINDS of share at a point (the ladder's rungs summed in energy, as a fraction of its top) and `seaBandShares` returns every band's separately; `seaSummary(sea, x, z)` returns `{ Hs, Tp }`: every band summed in energy, and the period of whichever is carrying the most of it there. `stormSeaAt` is the same reading for the ladder ALONE, which is what the renderer judges a crest against out there. The sim's `maxHs` column is `seaSummary`'s Hs at the craft's position.
...
**The axis is √d, not d**: row `i` sits at `(i·tableRoot)²` metres, `tableRoot` = 0.1 m^½, so the SPACING it gives is `2·tableRoot·√d` — eight centimetres where the bed breaks the surface, a metre at 25 m, twelve metres out on the abyssal plain. That is the shape of the question: every coefficient in the table moves with `k·d` and is flat once `k·d` is past π, so the resolution is wanted in the shallows and wasted in deep water. Measured against the exact functions over the water a course is ridden in, the worst interpolation error is 0.23 % (the old uniform tenth-of-a-metre axis gave 0.085 % there and 6.7 % at the `minDepth` floor, where this one gives 2.8 %).

The reach has to cover the DEEPEST water the model ever samples, because the read CLAMPS to the last row: past it a kilometre-long storm swell would be read at a depth it is not in, shoaled and given several times the orbital velocity it has. That depth is the open ocean's floor, `tableDepth` = 4 000 m — 633 rows on this axis, against 2 501 for a uniform axis reaching a sixteenth as far. The generator's own bed stops at −25 m.
...
| `stormRamp(out)` / `stormAt(bounds, x, z)`      | THE storm's ramp: the LADDER's own height profile, read straight between its rungs, as a fraction of its top (`STORM_HS`)                       |
| `coastAstern(out)` / `coastAsternAt(...)`       | the shorter ramp: `out / STORM_NEAR_REACH` clamped to 0..1 — how completely the coast is astern                                                 |
| `oceanDepth(bed, storm)` / `bedAt(level, x, z)` | the bottom out there: the rim's own ground falling away to `sea.open.depth`, on the storm's ramp                                                |
| `oceanWind(mean, shelter, astern)`              | the mean wind a point feels: the level's own under its shelter, both giving way to the storm as the coast falls astern                          |

**There are two ramps, and the split is the point.** The sea's height and the BED under it climb together on `stormRamp`, because the depth-limited clip (`breakingHs`·d) couples them and a bed that lagged the sea would flatten it. The coast's own sea and the WIND end together on `coastAstern`, because both are the coast's weather stopping rather than the ocean's building — and because inverting Pierson–Moskowitz at the top of the ladder asks for a 176 m/s wind, which the rider feels DIRECTLY through the aero term and which would blow the craft off the water before he saw any of the sea he rode out for. So past the first rung the swell is bigger than the wind over it could have built, which is exactly what a QUOTED sea is for.

**The sea out there is the STORM LADDER** — `sea.open.ladder`, a rung at a time. It is QUOTED rather than grown, the way a `SeaOverride` is, because past the rim the fetch law has nothing left to say: the coast the level is a piece of is kilometres astern, and the sea is whatever weather the game says is out at sea. Each rung is a height and the distance it stands at; `stormRamp` reads straight between them, so the ladder IS the height profile of the ocean.

The ladder is authored in DISTANCE, because distance is what the model has, but it was CHOSEN in time — how long a rider holds the throttle open before the sea reaches each height, at 25 m/s, the catalog's mean top speed. Every rung is 45 km and half an hour:

| Rung | Out     | Riding  | Hs      | Tp     | λ (peak) |
| ---- | ------- | ------- | ------- | ------ | -------- |
| rim  | 2 500 m | 1.7 min | 20 m    | 11.9 s | 222 m    |
| 1    | 45 km   | 30 min  | 100 m   | 26.7 s | 1 111 m  |
| 2    | 90 km   | 60 min  | 200 m   | 37.7 s | 2 222 m  |
| 3    | 135 km  | 90 min  | 400 m   | 53.4 s | 4 444 m  |
| 4    | 180 km  | 120 min | 600 m   | 65.3 s | 6 667 m  |
| 5    | 225 km  | 150 min | 1 000 m | 84.4 s | 11 111 m |

**A band per rung, not one band scaled.** A sea quoted by its height takes its period, and so its wavelength, from `steepness` (`periodForHeight`), so a single band laid at the thousand-metre top and scaled down to the twenty-metre sea a rider meets two kilometres out would deal him an eleven-kilometre wave with a fiftieth of its proper face — an ocean tilting, not a wave. Each rung carries its own height at its own steepness instead, and neighbouring rungs hand over on the height the way the coast's sea and the storm already do, so the wavelength walks UP the ladder with the height. The first rung is the twenty-metre storm the model was first sized to carry, kept where it was, so everything within a couple of kilometres of a level reads exactly as it did.

Each rung is laid over `sea.open.bandHigh` = 4.8 of its own peak rather than over `minPeriod` in absolute seconds. That floor would give the top rung a band 0.7–33.7 f_p — forty-eight times a coastal sea's frequency range on the same eight components — and it breaks on the cos² directional weight, which vanishes at the edge of the spread: when the longest component's draw lands out there its energy is normalised onto whatever is left, and across a band that wide that is a far shorter wave. Measured over the seed corpus it reached `a·k` 0.89 with 90 % of the band's energy in one component, a wave several times past breaking that the renderer paints entirely in foam; more components barely helped (0.49 at twenty-four), because the width is the fault, not the resolution. At 4.8 the worst is 0.462 — the twenty-metre storm's own long-standing figure, 0.461, to three decimal places.

A level already handed a bigger sea than a rung keeps it; a level with no wind gets no storm at all, the same rule that keeps `createSea` from building a spectrum on a wind of nothing.
...
carried = coast · (1 − astern)               what is left of it
target  = carried + openHs · storm           the height that should stand here
need    = √(target² − carried²)              the height the ladder has to make up
```

so Hs grows STRAIGHT from the coast's own sea to the storm's, with no dip where the two spectra cross — and because the coast is gone by the FIRST rung, every rung above it stands at its own authored metre and nothing else. Inside the level `astern` and `storm` are both 0, every open band's share is exactly 0, the whole ladder is skipped in one comparison, and the sum a coast's water returns is term for term the one it returned before there was an ocean beyond the rim.

**Which rungs carry `need`** is the second half. Each rung's own place on the ramp is its height over the ladder's top, by construction of `stormRamp`, so the height standing here sits between two rungs — and those two share it, by ENERGY (`f` and `1 − f` of it), which is what makes their combined height exactly `need` whatever their own quoted heights are. Below the first rung that rung takes all of it, which is precisely what the single open band did before there was a ladder.
...
- **The bed.** The clamped rim is thirty-odd metres of water, which the depth-limited clip (`breakingHs`·d) would hold a twenty-metre sea down to fifteen in; past a corner where the coast happened to reach the box it is a plateau of LAND standing out in the open ocean. `bedAt` falls the rim's ground away to `sea.open.depth` = 4 000 m over the STORM'S ramp — the same one the height climbs, which is what keeps the margin: the clip asks for `Hs/0.55` and the bed gives about twice it at every rung. A thousand-metre sea needs 1.8 km of water under it, and gets an abyssal plain.
...
| Past the rim | riding  | storm | depth     | ocean share | open share | wind at 2 m | Hs        | Tp     | λ dom   |
| ------------ | ------- | ----- | --------- | ----------- | ---------- | ----------- | --------- | ------ | ------- |
| 1 m          | 0 min   | 0.00  | 38.6 m    | 1.00        | 0.00       | 11.4 m/s    | 2.63 m    | 6.0 s  | 53 m    |
| 1 079 m      | 1 min   | 0.01  | 59.4 m    | 0.57        | 0.01       | 15.7 m/s    | 10.13 m   | 11.9 s | 206 m   |
| 2 203 m      | 2 min   | 0.02  | 94.7 m    | 0.12        | 0.02       | 20.1 m/s    | 17.94 m   | 11.9 s | 216 m   |
| 44 592 m     | 30 min  | 0.10  | 419.1 m   | 0.00        | 0.10       | 21.3 m/s    | 99.23 m   | 26.7 s | 1 071 m |
| 89 589 m     | 60 min  | 0.20  | 816.0 m   | 0.00        | 0.20       | 21.3 m/s    | 199.09 m  | 37.7 s | 2 136 m |
| 134 589 m    | 90 min  | 0.40  | 1 607.5 m | 0.00        | 0.40       | 21.3 m/s    | 398.17 m  | 53.4 s | 4 267 m |
| 179 588 m    | 120 min | 0.60  | 2 402.6 m | 0.00        | 0.60       | 21.3 m/s    | 598.17 m  | 65.3 s | 6 398 m |
| 224 588 m    | 150 min | 1.00  | 3 985.4 m | 0.00        | 1.00       | 21.3 m/s    | 996.34 m  | 84.4 s | 6 763 m |
| 269 588 m    | 180 min | 1.00  | 4 000.0 m | 0.00        | 1.00       | 21.3 m/s    | 1 000.0 m | 84.4 s | 6 763 m |

`riding` is at the ladder's own reference, 25 m/s. The steepness `H/λ` stays between 0.02 and 0.12 the whole way out — under Michell's 1/7 everywhere, which is what keeps the ocean water rather than a snowfield.

The `ocean` scenario stands the craft at the FIRST rung and `maelstrom` at the top; `make ride SCENARIO=ocean` (or `maelstrom`) rides one and `make screenshots SCENE=maelstrom` photographs it. A face a thousand metres high is wider than anything the renderer draws, so what the camera sees out there is the whole world tilting — which is why the scenario exists at all.

**What the RENDERER had to learn.** Every threshold that says how high or how steep a wave is standing was written against `sea.hsRef`, the coast's own swell — a crest tint, a frustum margin, the relative tilt bands, the whitecap and breaking gates. Judged against a one-metre coastal sea, a thousand-metre one is above every one of them at every vertex and comes out solid white. `pwa/src/game/water-mesh.ts` now reads `stormSeaAt` once a frame at the craft (the storm alone, both numbers 0 inside a level, so nothing a course is ridden over changes) and takes the bigger of the two seas for each threshold. The storm is uniform to a fraction of a percent across a mesh two hundred metres wide two hundred kilometres out, so once a frame is enough.
...
| `sea.tableRoot` / `tableDepth`       | 0.1 / 4000  | m^½, m | the per-component depth table, on a √d axis                       |
| `sea.steepness`                      | 0.09        | —      | Hs/L₀ of a QUOTED sea — its period, and so its wavelength         |
| `sea.open.ladder`                    | 6 rungs     | m, m   | THE STORM LADDER: how far past the rim, and the Hs standing there |
| `sea.open.bandHigh`                  | 4.8         | f_p    | the band a rung is laid over, as a multiple of its own peak       |
| `sea.open.wind`                      | 25          | m/s    | the wind out there at 10 m, reached at the ladder's FIRST rung    |
| `sea.open.depth`                     | 4000        | m      | the ocean floor out there, so nothing clips the storm             |
