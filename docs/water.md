...
**What it costs, and where.** A trail is at most `maxAge` (9 s) of sources — a couple of hundred at the roster's fastest — and every sample asks first whether it is inside the trail's box at all, so the grid's vertices nowhere near a wake pay a few compares. The buffer is indexed in chunks of sixteen slots, each with a box of its own, and since sources are laid in time order along one continuous line a chunk is a short stretch of trail: a sample near the hull asks the last chunk or two. Inside a chunk's box a source is one square root and one cosine-sine pair — the window is a polynomial bump with an edge, not a Gaussian — and the age-only half of every source (its radius, its faded amplitude, its carrier's phase, whether it is still young) is refreshed once per distinct `t` and shared by every sample of that step or frame; a ring faded to nothing is skipped in one compare. Measured on the medium WATER row's grid (6641 vertices) in Node: about 1.5 ms a frame on top of the sea's own sum while a wake is in the frame, most of it the fine core round the hull summing the dozen packets that stand there. `make wash` prints the microseconds a sample costs beside the heights.
