...
What comes back from one call (`surfaceAt(sea, level, x, z, t, out?)`, a `SurfaceSample`): the surface `height` (m against sea level), its unit normal `(nx, ny, nz)`, and the Lagrangian surface velocity `(vx, vy, vz)` (m/s, world frame): the phase-resolved Airy orbit plus phase-averaged Stokes drift and any current. `heightAt` is the height alone. Every formula below names its source; every number is quoted with its unit as `TUNING` states it.

## Building the sea (`createSea(level, seed, wind = level.wind)`)

The field is a sum of `TUNING.sea.components` = 8 components, each a linear (Airy) wave carrying **Stokes' second-order correction** (1847): `η = a·sin φ − ½·k·a²·cos 2φ`. That second term is the trochoidal shape — a peaked crest over a long flat trough — and taking it this way rather than as the Gerstner horizontal displacement (Tessendorf 2001; Finch, _GPU Gems_ 1 ch. 1) keeps the height a function of the **undisplaced** `(x, z)` the physics asks about: a Gerstner field would have to be inverted at every hull probe and every mesh vertex. Heights, slopes and phase-resolved orbital velocities are otherwise linear theory's; the mean transport adds the spectrum-resolved second-order Stokes velocity, which is what a freely drifting hull needs when the linear orbit closes. `TUNING.sea.crestSharpness` multiplies the height correction (1 is Stokes' own coefficient) and `crestMaxSteepness` is the `a·k` it is evaluated at, at most, since the expansion grows a second bump in the trough if pushed past its range.
...
2. **The headline numbers.** The significant height and the peak period at the reference fetch, for the mean wind `U` (m/s at 10 m):
   ...
   The wind sea's height and period then take `TUNING.sea.heightScale` = 1.5 and `periodScale` = 0.95 — the two arcade dials that say how big and how long a wind sea is _here_ against what the law alone grows. The fetch growth is a ratio, so neither disturbs its shape. Steepness goes as `heightScale / periodScale²`, so the two together are the whole of how a wind sea reads. **`periodScale` is also the big lever on how far apart the waves stand, and it is not free**: raising it to 1.15 spread the tightest seed's crests from 45 m to 60 and its encounter period from 1.4 s to 1.8 — the room a flight wants to land in — and took away the bow going under, because a longer wave travels faster (`c = √(gλ/2π)`) and a following sea that overtakes the hull more slowly stops burying it. Over the twelve-seed corpus with the bars held fully forward, eleven seeds put the hull under at 0.95 and one did at 1.15, so the room between the waves is bought from `bandHigh` instead. The pair below is still what stops the sea running away; pushing them to 1.8 / 0.85 (2.5× the natural steepness) made a sea the bot spent a third of every run airborne in, and `make sim` fell from 16/16 finished to 12/16.
...
3. **The components — FOUR BANDS (R28).** There are four kinds of wave a rider can meet and they do not come from the same weather, so the field is laid in four bands and every point takes a share of each.
   ...
   **The swell** (`sea.swell.components` = 3) is the sea this coast's own wind did NOT make — groundswell, radiated by weather a thousand kilometres away, sorted by frequency on the way (dispersion runs the long waves out in front) and arriving long, slow and nearly all one way whether or not there is any wind here. It is what makes surf on a calm morning, and without it every wave in the game is a locally grown wind sea: at the bottom of R12's band that is 1.2 m at a 4.2 s period, a 28 m wave under a 3.5 m hull, and it reads as texture rather than as ocean however much of it there is.

   So it is QUOTED rather than grown — there is no fetch here to grow it over — and everything about it is the opposite of the wind sea beside it. **How big it is is the LEVEL's** (R36, `Level.swell`): the generator deals a height out past the islands inside `SWELL_DIAL` = 1–20 m, log-uniformly over that band and squared toward its floor so the median shore gets about 2.1 m and one in eight more than ten, and a run may ask for any height in the band instead — which is the one knob that says how much OCEAN a level has in it, and the one sea a rider can be given without a wind to grow it. How much of it ARRIVES is the coast's own share (`Biome.sea.swell`). Its period follows from its height and `swell.steepness` the way the storm ladder's does, NOT from the wind; `swell.components` = 3 over `bandLow`..`bandHigh` = 0.93–1.08 of its own peak, a tenth of an octave, so the three beat into **sets** hundreds of metres long — three or four big waves and then flatter water, which nothing else in the model makes; and `swell.spread` = 0.16 rad (~9°) is a third of the wind sea's, because distance sorts direction as surely as it sorts frequency. Its share is an **exposure** of its own (`seaExposure` in `fetch.ts`) — the same fan the ocean band's is measured over, land cutting it the same way so that a river a hundred metres up has none of it, but aimed DEAD ONSHORE (`Level.seaHeading`) instead of up the wind. Which is the whole point of carrying it: the weather that made this sea is a thousand kilometres away and days old, so what decides how much of it stands at a point is the land in front of that point and nothing today's wind is doing. Read off the wind — which is what it used to read — a wind turned past the beam took a twenty-metre swell down with the wind sea it really does flatten. The field is a function of the LEVEL alone, so it is measured once per coast and cached (`createSea` runs per run and the fan is five sweeps of the grid). It carries a phase field, so it shoals and refracts: that is where a coast's big water outside the surf line comes from.

   `swell.steepness` = 0.035 is the most important dial here and it is the arcade's, not the ocean's. A real groundswell is about 0.008 — three metres over three hundred and fifty — and **it is invisible from a chase camera**: three metres over a hundred and seventy of horizontal run is a two-degree slope, and a rider a few metres above the water looking down his own wake sees a plane that slowly tilts. Quoted at nature's steepness the swell measured 34 m of crest length and read as nothing. At 0.035 a 3 m swell is 86 m long with a 7.4 s period — twice the wind chop's length, three times its height, and a face steep enough to see. The other end of the same scale is `sea.steepness` = 0.09, which is what the open ocean's storm is quoted at, and that is a wall.

   **And it is capped at a LENGTH, `swell.maxLength` = 220 m.** A fixed steepness means the wavelength grows with the height, and R36's dial reaches twenty metres: quoted at 0.035 that is a 570 m wave, which is the invisible-swell fault arriving again at the other end of the band — crest ahead and crest behind both off screen, and a plane that slowly tilts left on it. 220 m is where the two dials meet, being the length `sea.steepness` gives at the top of `SWELL_DIAL`, so the biggest swell a level can carry is quoted at the open ocean's own storm shape and a rider reads the same wall either side of the rim. Nothing under **7.7 m** touches it, which is every sea any level was dealt before R36 and a good way past it.

   **A flat calm has no swell the coast was DEALT.** `wind.speed === 0` gates that off, the same way the storm ladder is gated, and it is not a physical claim — a groundswell does not care what the local wind is doing, which is the point of it. It is that zero wind in this engine means a flat calm: the state every physics test stages its hull at rest on, and every turntable and rest scene is drawn over. R12 never deals a wind under 6 m/s, so no dealt level takes that branch.

   A swell the RUN ASKED FOR does not take it either (`Level.swellAsked`, set by the generator when `GenerateOptions.swell` was given). A rider who puts the WIND row on nothing and the WAVES row on twenty has asked for exactly the day R36 describes — a glassy morning with an ocean rolling under it — and handing him a mirror instead is the card lying about what it does. Nothing that stages a calm asks for a swell, so the harness keeps its flat water and the dial keeps its top end.
   ...
   **The open bands** (`sea.components` again, one per rung of `sea.open.rungs`) are the storm past the edge of the built level — [the open ocean](#the-open-ocean-past-the-rim-enginegameoceants) below, which owns the whole of it. Every one of their shares is 0 inside a level's bounds, so the coast's own water is untouched by them and the whole storm costs one comparison a sample: `surfaceAt` walks the field BAND BY BAND and skips a band whose share is nothing in a single test rather than once per component.
   ...
   All three bands are laid the same way. The ocean band's eight frequencies, log-spaced over the band `bandLow..bandHigh` = 0.7–2.0 × the peak `ω_p = 2π/Tp`, each owning the band between the midpoints to its neighbours. **The top of the band is under where JONSWAP's energy runs out, and that is the point**: what a rider counts is CRESTS, not energy, and a component at 2.4 f_p carries a few per cent of the sea while putting five more crests between every pair of the peak's. The number the ride answers to is the zero-crossing period `Tz = √(m0/m2)`, whose `m2` is almost all tail. Cutting at 2.0 leaves the peak and its face where they were and takes the chatter out from between them — `make sim`'s launches fell from 274 over the corpus to 194 with the air per launch an eighth HIGHER, what was lost being the hull thrown off chop it never aimed at:

   ```
   bandHigh = max( 2.0,  Tp / minPeriod )                    minPeriod = 3.0 s
   lo = 0.7·(bandHigh/0.7)^(i/8),  hi = 0.7·(bandHigh/0.7)^((i+1)/8)
   ω_i = ω_p · √(lo·hi),   Δω_i = ω_p · (hi − lo)
   ```

   `bandHigh` is a multiple of the PEAK, and a big sea's peak is slow: at a twelve-second peak, 2.0 f_p is still a six-second, fifty-metre wave, so a storm swell arrives with no wind chop on it at all — a mirror the size of a hill, which is the one thing a storm does not look like. So the band's short end is also held to an absolute shortest period and the wider of the two wins. A six-second wind sea is untouched (2.0 f_p is already shorter than 3.0 s); a swell gets the chop that rides on it. The floor moved up with `bandHigh` and for the same reason — a floor that lets a ten-metre wave in undoes the cut on every sea whose peak is slow. 3.0 s is a fourteen-metre wave: still the shortest thing in the field by a good margin, and still several cells of the water mesh at the craft.

> **2.0 and not lower, and the floor is a FEEL one.** The short waves are also what stuffs the bow in a following sea, and they stop doing it abruptly. Over the twelve-seed corpus with the bars held fully forward through the `following` scenario, eleven seeds put the hull under at 2.4, eleven at 2.0, eight at 1.85 and five at 1.7 — so the cut sits at the last value that costs the dive nothing at all, and the extra room 1.7 would buy is two metres of crest spacing on the tightest seed.
...
**What that gives, on one seed** (38, wind 13.4 m/s from 300°, `make waves --seed 38`; `make surf --seed 38` reads the same sea the way a rider meets it — crest spacing, how often a wave meets the bow, and where the white is):
...
That draw is the OCEAN's own, and it stands on top of the coast's (R36): `openHs` is the dealt storm and the level's own sea — its wind sea and its groundswell, already in quadrature — added the same way, because two seas standing in the same water add in energy. A shore dealt a metre of swell therefore meets the storm it always did (14.2 m becomes 14.25), and one dealt twenty metres meets a bigger one, so **riding out grows the sea whatever the shore was dealt and never shrinks it** — which is what the wave-size knob needs to stay a baseline rather than a ceiling.

### A band per rung, not one band scaled

A sea quoted by its height takes its period, and so its wavelength, from `steepness` — so a single band laid at the storm's full height and scaled down to the sea a rider meets halfway out would deal him the storm's wavelength at a quarter of its face: an ocean tilting, not a wave. The open band is therefore laid as several, at `sea.open.rungs` = 0.25, 0.5 and 1 of the way from the COAST's own sea up to the dealt storm, and neighbouring rungs hand over on the HEIGHT. From the coast's sea rather than from nothing, so no rung stands below the water a rider has just left and no two rungs can come out at the same height — which they would under a floor, and the handover divides by the gap between two rungs. No sea is ever drawn from a band more than twice its own height, so the wavelength tracks it to within that.
...
```
coast   = √((hsRef·exposure)² + (swellHs·seaward)²)   the coast's own sea here
...
## The tornado past the far edge (`engine/game/tornado.ts`), and the net that is its face
...
**And it is DRAWN.** `tornadoNetPlan(bounds, pace)` states the shape of that edge once — the level's own box and the distance every point of the edge stands outside it — and `pwa/src/game/edge-net.ts` walks it to stand a lattice of light there, lit where the hull is in it. The net is not a second boundary and has no physics of its own: it is the tornado's face, so what a rider can see IS what takes him. The plan is a **rounded rectangle** rather than a rectangle, because `oceanOffset` measures a corner as the hypotenuse of both axes; drawn square, the lattice would stand `edge · (√2 − 1)` — some 1 800 m at the shipped class — beyond its own tornado where two sides meet.
...
`tornadoRamp(out)` eases it in over `wind.tornado.band` = **49 m**, a second and a half of riding at the roster's best. Thin on purpose: the net is a thing a rider can see, and the tornado is what it does, so reaching the lattice has to BE the trigger. A band wide enough to be gentle carried him hundreds of metres past the thing he was looking at before the air turned on him. It is still a ramp and not a step — the air freshens over a second or so — but it is the net's own thickness. Inside a level, and anywhere in the storm short of the edge, it is exactly 0 — `tests/tornado_test.ts` holds it at 0 at every gate of every seed in the corpus.

**How much of it a rider gets is read off HOW HIGH HE IS**, and that is what makes the edge a hazard with a shape rather than a wall. `tornadoHeightGain(over)` is `tornado.deck` = 0.2 down on the water, rising to 1 over `tornado.reachUp` = 11 m — a real tornado's inflow is choked by friction in the first few metres over the sea and stands in full above that layer. It scales the **column** and not the inflow, and the split is the whole of the design:

- **Meet the net on the water** and the inflow is the whole of it: at full strength it stops the hull inside a couple of the net's own thicknesses, and carries it back toward the start. The column has no height to stand in and lifts almost nothing.
- **Meet it with height** — thrown up on a crest, or already flying — and the column has the hull at full strength and throws it. The sea out here swings a storm's whole height, so which of the two a rider gets is the SEA's to say and not a dice roll. `over` is measured against **mean** sea level for exactly that reason: a hull carried up on a crest is higher in the inflow layer even with water still under it.

**And at the very foot of the column the inflow turns up.** Air that has converged from every side has nowhere left to go, and `tornadoTilt(over)` is the depth of that turn: all of it at the water, none of it at `tornado.tiltUpTo` = 2 m. It rises at `tornado.tiltLift` = 1.35 × the hull's own `hoverSpeed` — quoted per craft so all four are lifted alike — and it works on the **deck** (`tornado.deckPlate` = 0.45 of the plan area) as well as on whatever is in the air, because a hull floating the right way up still shows a rising wind its topsides. Without it the twenty-metre case cannot exist: the only lift in the tornado is the column's, the column needs height, and a rider down on the water would be pushed home along the surface with the hazard reading as a current. With it he is put **up** and back, which both reads as a contact and hands him the height the column wants.

| What                                           | Answers                                                                                                          |
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `TORNADO_EDGE`                                 | how far past the rim it stands, m — derived off the storm's reach and the roster's top speed                     |
| `tornadoNetPlan(bounds, pace)`                 | the shape of the edge, for anything that has to DRAW it: the box, and how far outside it the net stands, m       |
| `tornadoRamp(out)` / `tornadoAt(bounds, x, z)` | how much of it stands there: 0 at the edge, 1 a `band` further out                                               |
| `tornadoHeightGain(over)`                      | how much of the COLUMN reaches a hull that high over mean sea level: `deck` at the water, 1 at `reachUp`         |
| `tornadoTilt(over)`                            | how much the inflow is still turning UP there: 1 at the water, 0 at `tiltUpTo`                                   |
...
Nothing here teleports a craft, resets a run, ends a run, or stands a wall in the water — the net is light and nothing else. The rider is turned back, or thrown twenty-odd metres up and a long way toward where he started, by a wind; he lands, and rides on; the course, the gate count and the clock are untouched, and he is free to ride straight back out and be thrown again. `tests/tornado_test.ts` holds all four of those.
...
| Term      | What it is                                                                                                                                 | Read off                                                              |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------- |
| **surf**  | the bed comes up, the wave cannot stand in the water it has left, and it falls over — the band off every shore, whatever the wind is doing | the DEPTH LOAD, `Hs / (breakingHs·d)`, opening at 0.4 and full by 0.9 |
| **crest** | out in deep water the top of a steep wave still spills down its own face — a few crests in a running sea                                   | the tilt band against the crest's height, 0.35–0.8 Hs                 |
| **caps**  | past a fresh breeze the wind blows the tops off                                                                                            | the wind THIS water feels, 10 m/s to 22                               |
...
v_s     = Σ ½·a²·ω·[coth²(k·d) + 1]·k⃗         (finite-depth surface Stokes drift, equal to
                                                 a²·ω·cosh(2kd)/(2·sinh²(kd))·k⃗)
v_x,v_z += flowAt(level.flow, x, z)             (R27: the water itself, where a river is running)
```

The Airy terms are the velocity at a fixed phase and average to zero over a cycle. `v_s` is the wave-induced difference between that Eulerian orbit and a water parcel's Lagrangian trajectory: it points along each component's local, refracted wave vector and is summed over the full directional spectrum. The finite-depth factor strengthens transport as the bed constrains the orbit. Cross-component interference is phase-dependent and averages away, so each component contributes its own spectral transport (Kenyon 1969; Liu et al. 2021).
...
The orbital velocity is what makes a wave face lift the bow and a crest surge under the hull; Stokes drift is what keeps transporting it after those orbits are averaged over time. The hull's drags are computed against the flow RELATIVE to the Lagrangian surface water (`hull.ts` subtracts the sample's `vx, vy, vz` from each probe's velocity), so an unpowered craft bobs with each passing component while acquiring the spectrum's net drift. A mesh of forty thousand vertices calls this once a vertex a frame; the `out` parameter is how it allocates nothing.

## The wash (`engine/game/wash.ts`)

Everything above is the sea the level brought. The WASH is the water the craft made — the V behind a moving hull, the rings a heaving one radiates, the ring a landing knocks up — and it is real water: `surfaceAt` sums it into every sample that carries the whole field, so the hull's probes read it at 120 Hz exactly as they read the swell, and the renderer's grid, displaced by the same function, draws the wave the hull felt. A rider crossing his own trail in a turn is lifted by it; a rival abreast is rocked by it; a wakeboard could, in principle, ride it.

**The model is Huygens'.** A hull moving through water is a line of point sources, and the V of a wake is nothing but their rings adding up. Each source is one ring packet:

```
η(d, τ) = a · (1 − e^(−τ/rise)) · e^(−τ/life) · √(birth/d) · (1 − (u/2·width)²)² · cos(k·d − k·(birth + c·τ))
u       = d − (birth + c_g·τ)
```

with `d` the distance from where the source was laid, `τ` its age, `birth` the radius the ring is born at (a hull half-length and a little — inside it there is no wave, because that water was the hull), the envelope going out at the GROUP speed `c_g` and the crests inside it at the PHASE speed `c`, and the amplitude thinning as 1/√d because a ring's circumference grows. ONE wavelength serves every source: the deep-water wave whose celerity is the hull's HUMP speed (`TUNING.wash.hump`, 3.06 m/s ⇒ λ = 2π·U²/g ≈ 6 m), because that is the wave a hull of this length is tuned to make and the one it sits in at the hump. A moving hull's rings then interfere into the V by themselves — narrowing as the speed rises past the celerity, the way a planing hull's wake does in the reference photographs (at high Froude number the observed wake angle closes as 1/Fn) — and a hull with no way on lays rings that simply spread. The slope, the surface's own rate of rise and Airy's horizontal orbit at the surface (`ω·η` along the ring's travel) come with the height, so a hull on a ring is lifted, tilted and carried by it.

**What strikes a source** is three readings off the craft, summed into one amplitude and laid into the rider's own trail by `stepWash` right after `stepCraft`:

| Reading     | When it is laid                            | Worth                                                                                                                                                                                                                                |
| ----------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| THE PASSAGE | one source every `spacing` (1 m) of travel | `crest · min(1, U²/hump²) · (1 − (1 − planingShare)·planing) · wetted` — rising with the square of the speed to the hump, then falling to a fraction as the hull comes up on the plane                                               |
| THE BOB     | every `every` (12) steps, ten a second     | `bob · (waterVy − vy) · wetted`, SIGNED and capped at `bobMax` — a hull going down shoves a crest out, a hull coming up draws a trough, so a heaving hull radiates a train at its own period; a hull on a flat calm radiates nothing |
| A SPLASH    | at once, on every `land` and `dive` event  | `splash ·                                                                                                                                                                                                                            | descent | `, capped at `maxCrest` |

A source under `minCrest` is not laid at all, so a hull lying on a flat calm costs the sea nothing; one past `maxCrest` is capped there.

**What the hull feels of its own.** Inside the birth radius of any source there is no wave, ever. And a source younger than `young` (2 s) stands nowhere inside the footprint of the hull that laid it: a ring is a full circle from its birth, so the forward arc of one laid a hull length ago is standing where the hull now is — under it, not in the water — and a hull that read it rode up on its own fresh wake, which is how the first cut of this model heaved itself to two metres at the hump. The mask is the OWNER's alone: a rival passing over the same water feels every ring, and the hull that laid them feels them again the moment it turns back across its trail. What it does feel of its own at once is the SQUAT: at the hump the coherent trough of the dozen sources behind it, which is the wave a displacement hull sits in.

**What it costs, and where.** A trail is at most `maxAge` (9 s) of sources — a couple of hundred at the roster's fastest — and every sample asks first whether it is inside the trail's box at all, so the grid's vertices nowhere near a wake pay a few compares. The buffer is indexed in chunks of sixteen slots, each with a box of its own, and since sources are laid in time order along one continuous line a chunk is a short stretch of trail: a sample near the hull asks the last chunk or two. A sample that gets past the trail's box does not walk those chunks itself, though — it reads a list GATHERED once for the patch of water it stands in (`gatherAt`, `GATHER` = 3 m, about a hull's length). That is there because the deciding, not the adding up, was nine tenths of what the wash cost: on a twelve-craft race a sample walked 434 slots to find the hundred worth a cosine, and a hull asks once per buoyancy probe, so the same walk was repeated two dozen times for one hull standing in one place. The gathered list is a superset walked in the trail's own order and every sample still runs the same tests on it, so the sum is added up in the same order and a run replays to the bit. Inside a chunk's box a source is one square root and one cosine-sine pair — the window is a polynomial bump with an edge, not a Gaussian — and the age-only half of every source (its radius, its faded amplitude, its carrier's phase, whether it is still young) is refreshed once per distinct `t` and shared by every sample of that step or frame; a ring faded to nothing is skipped in one compare. Measured on the medium WATER row's grid (6641 vertices) in Node: about 1.5 ms a frame on top of the sea's own sum while a wake is in the frame, most of it the fine core round the hull summing the dozen packets that stand there. `make wash` prints the microseconds a sample costs beside the heights.

**What it is not.** Not the Kelvin pattern proper: one wavelength rather than a dispersive spectrum, so the transverse train behind a displacement hull is short and the V is a Mach cone rather than a 19.47° wedge — which is the honest shape for a planing hull and the cheap one for the grid. Not in the far water: a far-grid sample of the longest components (`surfaceAt`'s `count`) carries none of it, because a six-metre ring is not a swell its cells can hold. Not on the beach: it fades out under `shoal` (1 m) of water. And not presentation: the renderer's wake map (`pwa/src/game/wake.ts`) restates none of it, carrying the white, the churn and only the relief that stands inside the hull's own footprint — the transom's hollow and mound, a landing's crater — while the arms of the V, a splash's ring and a bob's ring are the wash and the wash alone.
...
| ------------------------------------ | ------------ | ------ | ----------------------------------------------------------------- |
| `sea.components`                     | 5            | —      | WIND-SEA band components — the FRAME's budget sets it             |
| `Level.swell` (`SWELL_DIAL`)         | 1–20         | m      | the GROUNDSWELL out past the coast — the level's, not this file's |
| `sea.swell.steepness` / `steepVary`  | 0.035 / 0.25 | —      | how steep it is quoted at, and so how LONG — the arcade's dial    |
| `sea.swell.maxLength`                | 220          | m      | ...and the length it may not pass, which is what caps R36's dial  |
| `sea.swell.components`               | 3            | —      | ...and how many, over a tenth of an octave: what makes SETS       |
| `sea.swell.spread` / `off`           | 0.16 / 0.44  | rad    | its own fan (~9°), and how far its heading sits from the wind's   |
...
| `sea.bandLow` / `bandHigh`           | 0.7 / 2.0    | × ω_p  | the band the components are laid over                             |
| `sea.minPeriod`                      | 3.0          | s      | absolute short end of the band — the chop on a slow swell         |
...
The level contributes `wind.speed` (6–14 m/s, R12) and `wind.from` (always off the sea — R12, which is what puts the ocean upwind of the whole coast), its `offshore`, `ground` and `flow` fields, `river.discharge` (120–600 m³/s, R27), and `water.density` (1005 kg/m³ on the taiga coast — brackish — 1024 on the mangrove, and 1027 on the arctic and the karst; R13), which is the density every hydrostatic and hydrodynamic force uses.

## What holds it

`tests/waves_test.ts`: the dispersion relation in both limits and everywhere between; the shoaling coefficient's growth; the phase field (the wavelength the field carries against the depth's own at every gate of the shared corpus and over the exposed water, and crests turning toward the shore off an oblique wind); the fetch law and its PM cap; R28's two bands (the ocean's sea reaching the shore undiminished, the river getting the wind's chop and none of the ocean's); finite-depth Stokes transport and the idle craft it carries; R27's current running down the channel and quickening as the banks close; the depth-limited clip in the shallows; bounded heights everywhere; purity (the same point at the same time is the same surface). `tests/wind_test.ts`: the profile, the shelter, the gusts, and the wind freshening into the storm past the rim. `tests/collision_test.ts`: the bounds holding at a land rim and letting a rider out at an open one. `tests/determinism_test.ts`: a run replays. **`make waves SEED=`** (`scripts/waves-lab.mjs`) is the lab: a transect from the shore out to sea at several moments, Hs against offshore distance, the spectrum, a walk OUT INTO THE OPEN OCEAN past the level's rim and a walk UP THE RIVER (the two other kinds of water a rider can reach), and a table of the two bands' shares, the local wind, Hs, Tp, wavelength, steepness H/λ, celerity, the depth ratio d/λ with the regime it puts the wave in (deep / intermediate / shallow), and the depth a wave breaks at — required before and after any change to `water.ts`, because a wave model is judged by the sea it makes and a screenshot shows one wave.
...
Tessendorf, _Simulating Ocean Water_ (2001) · Finch, "Effective Water Simulation from Physical Models", _GPU Gems_ 1 ch. 1 (2004) · Hasselmann et al., JONSWAP (1973) · Pierson & Moskowitz (1964) · _Shore Protection Manual_ (SPM, 1984) fetch-limited growth · Longuet-Higgins, Cartwright & Smith, directional spectra (1963) · Fenton & McKee, "On calculating the lengths of water waves" (1990) · Dean & Dalrymple, _Water Wave Mechanics for Engineers and Scientists_ (1991) · Kenyon, "Stokes Drift for Random Gravity Waves" (1969) · Liu et al., "Observational Evaluation of Global Wave Effects" (2021) · McCowan (1894) · Charnock (1955) · Stull, _An Introduction to Boundary Layer Meteorology_ (1988) · IEC 61400-3.
