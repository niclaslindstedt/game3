...
npm run sim -- --max 300                  # give a run longer than 240 s to finish
npm run sim -- --json examples/sim-report.json   # the rows, events dropped
npm run sim -- --help                     # every flag with its default
```

The default seeds are the ones `examples/seeds.md` describes, so the table CI prints is a table somebody has looked at the plans of. The run cap is `--max` = 240 s of sim time per run (the harness's own default, and what `tests/simulation_test.ts` gives a generated seed; `make sim ARGS="--max 300"` passes anything else through). An unknown flag exits 2 (`scripts/lib/cli.mjs`).
...
The slowest row is the otter on seed 123 at 227.9 s with four resets — inside the cap, and the reason the cap is 240 s rather than 120: a touring hull that runs wide onto a shore twice on a 2 km course is a slow run, not a broken one.
