...
//       the course by is one mouth and not three. AND IT NEVER MEETS
//       ITSELF: a river is not a road, two reaches cannot pass over and
//       under each other, and a loop that closes on its own water is one
//       the water has already cut off — the neck breaks and what is left
//       stands BESIDE the channel as an oxbow. So two reaches have to
//       stand further apart than the water they each carry plus
//       `river.selfBank` of neck, measured between reaches `river.selfSpan`
//       of a hairpin apart along it, and a walk that cannot is redrawn. It
//       CAN, because the meander turns at a circle the channel allows:
//       `river.bendWidths` of the half-width there, never under
//       `river.radius`, so the bends open out with the water and a reach a
//       hundred metres across does not fold. Which is also why the taper
//       is read off how far INLAND the water has got and not off how far
//       it has walked — a river thins as its catchment shrinks, so a reach
//       that spends three hundred metres on one loop comes out of it the
//       width it went in. WHAT KIND OF RIVER IT IS
